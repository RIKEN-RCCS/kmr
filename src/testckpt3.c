...
main(int argc, char **argv)
{
    ...
    MPI_Info_set(info, "ckpt_selective", "1");
    ...
    // HINT ON SELECTIVE
    //
    // If this kvs (kvs_points) is not saved to a ckpt file,
    // the result reported by restart process is not correct.
    // The reason is that this kvs is referred in every iteration, and
    // once restarted, whole kvs is lost if the ckpt file is not exist.
    kmr_map_once(kvs_points, (void *)&kmeans, kmr_ckpt, 0, generate_points);
	...
	// HINT ON SELECTIVE
	//
	// A ckpt file is taken for recording progress.
	kmr_replicate(kvs_cluster, kvs_all_clusters, kmr_ckpt);
